...
public class ProductService {
    ...
    public Product getProductById(Long id) {
        ...
        if (product.isEmpty())
            throw new RuntimeException("No Product Found with ID %d".formatted(id));
        return product.get();
    }
    ...
    public Product updateProduct(ProductDTO productDto) {
        if (productRepo.findById(productDto.id()).isEmpty())
            throw new RuntimeException("No Product Found with ID %d".formatted(productDto.id()));
    ...
    public void deleteProduct(Long productId) {
        if (productRepo.findById(productId).isEmpty())
            throw new RuntimeException("No Product Found with ID %d".formatted(productId));
